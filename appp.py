...
# Unified ML-Driven Fraud Detection for Retail POS (business-ready, single file)
...
# ---------------- 3) Labeled outcome (for training) ----------------
# Probability uses the same real-world drivers the model will learn; then we sample an outcome.
...
# ---------------- 4) Train the ML model and get fraud_score ----------------
...
X,y=df[features],df["fraud_flag"]; Xtr,Xte,ytr,yte=train_test_split(X,y,test_size=0.25,random_state=42,stratify=y)
...
# Logistic Regression: fast, stable, explainable; outputs calibrated probabilities
...
best_idx=int(np.argmax(f1)); best_thr=(thr[best_idx] if best_idx<len(thr) else 0.50)
ypred=(yprob>=best_thr).astype(int)
acc=accuracy_score(yte,ypred); pr=precision_score(yte,ypred); rc=recall_score(yte,ypred)
f1s=f1_score(yte,ypred); roc=roc_auc_score(yte,yprob); pr_auc=auc(rec,prec)

# ---------------- 6) Clear business summary ----------------
print("\nUNIFIED ML-DRIVEN FRAUD DETECTION FOR RETAIL POS")
print("- Transactions: 4,000 across Online and In-Store; payments reflect each channel’s reality.")
print("- fraud_score: probability an order is fraudulent (ML output from Logistic Regression).")
print(f"- Best threshold (max F1): {best_thr:.2f}")
print(f"  Accuracy={acc:.2%}  Precision={pr:.2%}  Recall={rc:.2%}  F1={f1s:.2%}  ROC-AUC={roc:.2%}  PR-AUC={pr_auc:.2%}")
print("\nStrong features (used in real-time decisions):")
signals=[
 ("addr_mismatch","Shipping vs Billing mismatch"),
 ("payment_risky","PayPal (online) or Gift Cards (in-store)"),
 ("high_amount_cat","Spend above 90th percentile for that category"),
 ("price_high_anom","Unusually high price (possible fraud)"),
 ("price_low_anom","Unusually low price (discount abuse)"),
 ("express_high_amt","Express shipping + high amount"),
 ("return_whiplash","Multiple recent returns"),
 ("oversell_flag","Quantity > 60% of available stock"),
 ("hoarding_flag","Bulk purchase (≥4 items)"),
]
for k,v in signals: print(f" • {k}: {v}")
print("\nHow threshold is determined:")
print(" • We compute performance across all cut-offs and select the threshold that maximizes F1,")
print("   balancing fraud capture (recall) and alert quality (precision). Business can still tighten")
print("   or loosen this value seasonally to meet risk or CX goals.")

# ---------------- 7) Real-time scoring helper ----------------
def score_new_order(order:dict)->float:
    """Return fraud_score ∈ [0,1] for a single order dict."""
    r=pd.DataFrame([order])
    # derive the same context features (category-aware)
...
# Example: leadership can test a new order instantly
example=dict(channel="Online",sku_category="Electronics",payment_method="paypal",
             ...
             log_amount=np.log1p(1800),express_shipping=1,returns_30d=0,on_hand=80)
fs=score_new_order(example)
print(f"\nInstant example → fraud_score={fs:.2f}  Decision at {best_thr:.2f}: {'ALERT' if fs>=best_thr else 'PASS'}")

# Optional: export predictions for dashboard use
pd.DataFrame({"fraud_score":yprob,"is_alert":(yprob>=best_thr).astype(int),"actual":yte.values}).to_csv(
    "predictions_latest.csv", index=False)
print("\nFile written: predictions_latest.csv")
