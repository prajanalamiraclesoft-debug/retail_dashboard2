...
with st.sidebar:
    ...
    st.caption("**Production decisions stay fixed at 0.30.** Slider is only for exploring impact.")
...
df["coup_pct"] = (df["coup"]/den).fillna(0); df["gift_pct"] = (df["gift_amt"]/den).fillna(0)
...
# strong fraud shapes (for recall protection)
...
def score_all(D):
    ...
    return clf.predict_proba(pd.concat([Xn_,Xc_],axis=1))[:,1]

df["score"] = score_all(df)

# ───────── Decisions @ fixed threshold (precision ↑, recall protected) ─────────
...
low_risk = (
    ...
    (df["age"]>=180) & (df["coup_pct"]<0.10) & (df["gift_pct"]<0.20) &
    (df["pay"].isin(["bank_transfer","cod","debit_card"]))
)

df["alert"] = np.where(force_alert, 1, np.where(low_risk, 0, (alert_score & risk_gate).astype(int)))
...
st.caption("No calibration. Class-weighted model + risk gate + low-risk suppression + strong-pattern force-alerts.")
