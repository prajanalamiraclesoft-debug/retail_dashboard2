...
from sklearn.model_selection import StratifiedKFold  # kept for parity with your imports
...
RANDOM_STATE = 42
PROD_THRESHOLD = 0.30  # business-fixed point
...
with st.sidebar:
    ...
    # Fixed prod threshold, plus a simple slider for "what-if" viewing
    TH = st.slider("Decision threshold (what-if view)", 0.01, 0.99, PROD_THRESHOLD, 0.01)
    st.caption("**Production decisions stay fixed at 0.30.** "
               "This slider only lets business users see how results would change.")
...
# class-weighting (boost recall at fixed cut)
pos = max(1, int((y==1).sum())); neg = max(1, len(y)-pos)
w_pos = min(10.0, neg/pos)

@st.cache_resource(show_spinner=False)
def fit_final_model(X, y, w_pos, max_iter=300):
...
def score_all(D):
    ...
    return final_clf.predict_proba(X_)[:,1]

df["score"] = score_all(df)

# fixed-threshold decisions + rule refinements
alert_score = (df["score"] >= TH).astype(int)       # what-if threshold from sidebar
force_alert = (df["s_any"] == 1)                    # recall booster
safe = (                                          # precision booster
...
st.caption("No calibration/optimizers shown. Model uses class weighting + strong-pattern overrides (recall) and safe-case suppression (precision).")
