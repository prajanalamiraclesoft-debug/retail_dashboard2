...
with st.sidebar:
    ...
    st.caption("The app adapts to the given tables and uses the threshold above.")

# ─────────────── BigQuery + loaders (schema-aware) ───────────────
...
def load(pred_t, feat_t):
    ...
    if fcols:
        ...
        for c in strong: sel.append(f"s.{c}" if c in fcols else (f"CAST(NULL AS INT64) AS {c}" if c=="fraud_flag" else f"CAST(0 AS INT64) AS {c}"))
    else:
        for c in strong: sel.append(f"CAST(0 AS INT64) AS {c}" if c!="fraud_flag" else "CAST(NULL AS INT64) AS fraud_flag")
...
if labs:
    lab = st.selectbox("Ground truth (1=fraud, 0=legit)", labs, index=0); y = df[lab].fillna(0).astype(int).values
    st.caption(f"Using label column: `{lab}`")
else:
    st.warning("No label column; using current decisions as proxy."); y = df["is_alert"].values
...
cat = c1.selectbox("SKU category", cats); pay = c2.selectbox("Payment method", ["credit_card","debit_card","gift_card","paypal","apple_pay"]); qty = c3.number_input("Quantity",1,99,1)
c4,c5 = st.columns(2)
price = c4.number_input("Unit price",1.0,10000.0,100.0,1.0); ship = c5.selectbox("Shipping country", ships); ip = st.selectbox("IP country", ips)

PAY_W = {"gift_card":0.35,"paypal":0.20,"apple_pay":0.12,"credit_card":0.10,"debit_card":0.08}
if "order_amount" in df and "sku_category" in df:
    cs = df[["sku_category","order_amount"]].dropna().groupby("sku_category")["order_amount"].agg(cat_mean="mean",cat_p90=lambda s: float(np.nanpercentile(s,90))).reset_index()
else:
    cs = pd.DataFrame({"sku_category":cats or ["apparel"],"cat_mean":[500.0],"cat_p90":[1500.0]})

def score(cat, pay, qty, price, ship, ip):
    r = cs[cs.sku_category==cat]; r = r.iloc[:1]
    amt = qty*price; geo = ship!=ip; high = bool(amt >= float(r.cat_p90)); w = PAY_W.get(pay,0.10)
    s = 0.08 + (0.35 if geo else 0) + (0.30 if high else 0) + w
