...
def load_df(PT,FT,S,E):
  ...
  j=bq.query(sql,job_config=bigquery.QueryJobConfig(query_parameters=[bigquery.ScalarQueryParameter("S","DATE",str(S)),bigquery.ScalarQueryParameter("E","DATE",str(E))])); d=j.result().to_dataframe()
  if "timestamp" in d: d["timestamp"]=pd.to_datetime(d["timestamp"],errors="coerce"); return d

df=load_df(PT,FT,S,E)
if df.empty: st.warning("No rows in this window."); st.stop()
df["fraud_score"]=pd.to_numeric(df["fraud_score"],errors="coerce").fillna(0.0); df["is_alert"]=(df["fraud_score"]>=TH).astype(int)
df["day"]=pd.to_datetime(df["timestamp"]).dt.normalize()  # <-- fix: datetime, not Python date
...
st.subheader("Daily trend"); tr=df.groupby("day").agg(n=("order_id","count"),alerts=("is_alert","sum")).reset_index()
st.altair_chart(alt.Chart(tr.melt("day",["n","alerts"],"series","count")).mark_line(point=True).encode(x="day:T",y="count:Q",color="series:N",tooltip=["day:T","series:N","count:Q"]).properties(height=260),use_container_width=True) if len(tr) else st.info("No activity.")

st.subheader("Fraud-score distribution"); h=alt.Chart(df).mark_bar().encode(x=alt.X("fraud_score:Q",bin=alt.Bin(maxbins=50),title="Fraud score"),y=alt.Y("count():Q",title="Count"),tooltip=[alt.Tooltip("count()",title="Count")]).properties(height=200)
...
def prev(cols,title):
  ...
  dd=pd.DataFrame([{"signal":c,"% in alerts":z.loc[z.is_alert==1,c].sum()/a,"% in non-alerts":z.loc[z.is_alert==0,c].sum()/na} for c in cols]).sort_values("% in alerts",ascending=False).melt("signal","group","value")
...
if C: co=df[["fraud_score"]+C].apply(pd.to_numeric,errors="coerce").fillna(0).corr().loc[C,"fraud_score"].reset_index().rename(columns={"index":"signal","fraud_score":"corr"}); st.altair_chart(alt.Chart(co).mark_bar().encode(x="corr:Q",y=alt.Y("signal:N",sort="x",title=None),tooltip=[alt.Tooltip("corr:Q",format=".3f")]).properties(height=130),use_container_width=True)
else: st.info("No pricing/inventory columns to correlate.")

st.subheader("Top alerts"); cols=[c for c in ["order_id","timestamp","customer_id","store_id","sku_id","sku_category","order_amount","quantity","payment_method","shipping_country","ip_country","fraud_score"] if c in df]; st.dataframe(df.sort_values(["fraud_score","timestamp"],ascending=[False,False]).loc[:,cols].head(50),use_container_width=True,height=320)
...
st.altair_chart(alt.Chart(cl).mark_rect().encode(x="Predicted:N",y="Actual:N",color=alt.Color("Count:Q",scale=alt.Scale(scheme="blues")),tooltip=["Actual:N","Predicted:N","Count:Q"]).properties(height=190)+alt.Chart(cl).mark_text(baseline="middle").encode(x="Predicted:N",y="Actual:N",text="Count:Q"),use_container_width=True)

try: A=roc_auc_score(y_true,y_score)
except: A=float("nan")
fpr,tpr,_=roc_curve(y_true,y_score); st.altair_chart(alt.Chart(pd.DataFrame({"fpr":fpr,"tpr":tpr})).mark_line().encode(x=alt.X("fpr:Q",title="False Positive Rate"),y=alt.Y("tpr:Q",title="True Positive Rate"),tooltip=[alt.Tooltip("fpr:Q",format=".3f"),alt.Tooltip("tpr:Q",format=".3f")]).properties(height=210,title=f"ROC (AUC={A:.3f})")+alt.Chart(pd.DataFrame({"x":[0,1],"y":[0,1]})).mark_rule(strokeDash=[4,4]).encode(x="x",y="y"),use_container_width=True)

P,R,_=precision_recall_curve(y_true,y_score); AP=auc(R,P); st.altair_chart(alt.Chart(pd.DataFrame({"recall":R,"precision":P})).mark_line().encode(x="recall:Q",y="precision:Q",tooltip=[alt.Tooltip("recall:Q",format=".3f"),alt.Tooltip("precision:Q",format=".3f")]).properties(height=210,title=f"Precision–Recall (AP≈{AP:.3f})"),use_container_width=True)
