...
# Unified Fraud, Pricing & Inventory Risk Detection Dashboard (BigQuery-backed)
# - "Raw data (sample)" → renamed to "Retail transactions" and does NOT show fraud_score
# - Decision threshold sits on the RIGHT of the header with a clear explanation tooltip
# - Fraud score distribution is explained and shows a vertical line at the current threshold
# - Strong signals section explains what the table/charts mean in plain business terms
# - No "geo" wording anywhere; we use "Shipping and IP countries differ" instead
...
def load(pred_t, feat_t):
    ...
    base = [
        "order_id","timestamp","fraud_score","customer_id","store_id",
        "sku_id","sku_category","order_amount","quantity","payment_method",
...
# ------------------------- Header with threshold on the RIGHT ---------------------
...
# Compute alerts AFTER setting threshold
...
# ------------------------- Retail transactions (no fraud_score here) -------------
...
# ------------------------- Fraud score distribution (with explainer + rule) ------
st.subheader("Fraud score distribution")
st.caption("Histogram of model scores across all orders. Right-side bars indicate higher risk. "
           "The dashed vertical line shows the current decision threshold.")
...
# ------------------------- Strong signals driving alerts (explain clearly) -------
st.subheader("Strong signals driving alerts")
st.caption(
    "These are patterns the system monitors. The charts show how often each pattern appears "
    "in flagged (suspicious) vs cleared (not fraud) orders—helping explain why items are flagged."
)
...
def prevalence(cols, title):
    ...
    rows = [{"signal":c, "% in flagged": z.loc[z.is_alert==1, c].sum()/a,
                       "% in cleared": z.loc[z.is_alert==0, c].sum()/na}
            for c in cols]
    dd = (pd.DataFrame(rows)
          .sort_values("% in flagged", ascending=False)
...
with left:
    prevalence([
        "strong_tri_mismatch_high_value",     # high value + multiple inconsistencies
        "strong_high_value_express_geo",      # express + high value + location inconsistency
        "strong_burst_multi_device",          # bursty activity / multiple devices
        "strong_price_drop_bulk",             # big price drop + bulk quantity
        "strong_giftcard_geo",                # gift card + location inconsistency
        "strong_return_whiplash",             # frequent returns pattern
        "strong_price_inventory_stress",      # price behavior + low inventory stress
        "strong_country_flip_express"         # country switch + rush
...
# ------------------------- ML-detected suspicious orders (with score) -----------
...
st.dataframe(
    df[df["is_alert"]==1].sort_values(["fraud_score","timestamp"], ascending=[False, False])[sus_cols],
    use_container_width=True,
    height=min(720, 28*min(len(df), 20) + 120)
)

# ------------------------- Model evaluation (optional, only if label exists) ----
st.subheader("Model evaluation")
labs = [c for c in ["fraud_flag","is_fraud","label","ground_truth","gt","y"] if c in df.columns]
if labs:
    y    = df[labs[0]].fillna(0).astype(int).values
...
# ------------------------- Instant decision (plain-language signals) -----------
...
def score_one(cat, pay, qty, price, ship, ip):
    ...
    amt = qty * price
    billing_shipping_diff = (ship != ip)           # <- clear wording
    high_for_category     = bool(amt >= float(row.cat_p90))
    # base fraud score logic
    ...
    return s, {
        "amount": amt,
        "mean": float(row.cat_mean),
        "p90":  float(row.cat_p90),
        "billing_shipping_diff": billing_shipping_diff,
        "high_for_category":     high_for_category
    }
